...
lemur.data=read.table('/media/elinambinina/DATA/Mes documents/e2m2Statistics2020/Lemur data.txt', head=T)
...
hist(lemur.data$taille, col='grey')
boxplot(lemur.data$taille, ylab='Height (cm)')

# Distribution of explanatory variables
par(mfrow=c(2,2))
hist(lemur.data$age, col='grey', main="Age") ; plot(lemur.data$sexe, col='grey', main="Gender") ; hist(lemur.data$GIparasites, col='grey', main="GIparasites") 
plot(lemur.data$malaria, col='grey', main="Malaria") ;
pairs(lemur.data)
...
plot(lemur.data$age, lemur.data$taille, pch=19)
m1=lm(taille~age, data=lemur.data)
abline(m1, xlim=c(0,35))
summary(m1)

# Relationship between height and GI parasites (univariate analyses)
plot(lemur.data$GIparasites, lemur.data$taille, pch=19)
m2=lm(taille~GIparasites, data=lemur.data)
abline(m2)
summary(m2)

# Relationship between height and gender (univariate and bivariate analyses)
boxplot(lemur.data$taille~lemur.data$sexe)
plot(lemur.data$age, lemur.data$taille, col=c('blue','red')[lemur.data$sexe],pch=19)
abline(lm(lemur.data$taille[lemur.data$sexe=='Female']~lemur.data$age[lemur.data$sexe=='Female']), col='blue',lwd=2)
abline(lm(lemur.data$taille[lemur.data$sexe=='Male']~lemur.data$age[lemur.data$sexe=='Male']), col='red',lwd=2)
...
hist(lemur.data$GIparasites, col='grey')
plot(lemur.data$sickGIparasites, col="grey", main="Infection status")
