...
## Cara Brook, 2016
## with code and data from Brook et al. 2016. Epidemics.
...
## Previously, we've fit models to time series of case counts (i.e. measles in 
## Niger, Epidemic Cards), but this time, our data are different. What are the states
...
## It's hard to tell which line better represent the data. Let's comapre the 
## likelihoods via a likelihood ratio test.

m1 <- loglikpc(par = out.pho$par, data = dat.pho, cate = c(0,15,30))
m2 <- loglikpc(par = out.pho2$par, data = dat.pho, cate = c(0)) 
## LRT
2*(m1-m2)

## Returns -1.156. Likelihood is chi-squared distributed, so we can apply a
## p-value to this estimate. p=XXX which is not significant. Might as well 
## keep the simpler, 1-age-class model. 
...
m3 <- loglikpc(par = out.eliza$par, data = dat.eliza, cate = c(0,15,30))
m4 <- loglikpc(par = out.eliza2$par, data = dat.eliza, cate = c(0)) 

## LRT
2*(m3-m4)

## This time, LRT = -15.63263. If we look at a chi-squared table,
## this would be assigned a p-value of p=XXX, meaning the added complexity
...
## Compare formally with 3 age classes with no clearance (m3)
m5 <- loglikpc.clear(par = out.eliza3$par, data = dat.eliza, cate = c(0,15,30))

## LRT
2*(m3-m5)

## LRT gives 3.51 which yields a p-value of p=XXXX, meaning that including 
## clearance creates a model with a significantly better fit to your
## data than not.

## You've just fitted, evaluated, compared, and selected models from data
## like that which you collected on Tuesday. But, remember that you had to
## have the model in mind when collecting these data. What if you hadn't 
## collected information on age?!? None of this would have been possible.
