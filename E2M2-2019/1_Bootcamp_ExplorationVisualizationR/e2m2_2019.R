...
##   and the each bat was weighted (Weight, in gramm), sexed (Sex, f/m), and aged.
...
##   for this tutorial ("tuto_e2m2.R" and "e2m2_FB.csv") to the created folder.
##   Note that the symbol "#" is used to put note within your code. Anything 
##   placed after this symbol in a row will not be execute as code, and are greened.
#############################################################################
...
######  An alternative to creating R project file (OPTIONAL).
##  Do not run the next two lines of codes if you have set an R project file.
##  Set your own directory
##  note that both the next two line codes work the same way \\ or / the path of your file
...
## Just add an exlamation mark "?" before it.
...
## You can also use summary() function. Not good for a big data set with too many variables.
...
## Now, we are trying to factor the Sex Variable:
...
## Try to remember thes function as they will be used in the following step
...
## Look at R data environment before running the code.
...
##########    Boxplot and histogramm functions produce nice graphic.
...
# You chack out details about capability of boxplot functio.
...
hist(Para,probability = T)

# Polished graph

hist(Para,probability = T,
     main="Parasite load distribution",
     xlab="Parasite number")
...
# For example, the Monthly parasite load mean Parasite.
...
# plot() function will chose default parameter according to you variable type
# Here it uses point since your variables have numerous datapoint and it is not grouped.
# Let us try anothe set of variable

plot(Weight~Sex)

# Do you see, plot() uses boxplot since you are using a catégorical variable against numerical.
# It automaticaly groupe your graph
# Try différent set of variables
...
# Try to play with option available int plot()function
...
# First get fit a curve to our data, you learn this later in other lectures
# For now we will just use a lowess fitting, a non linéar fitting.
...
ggplot(e2m2,aes(Age,Forearm)) +
  ...
  ggtitle("Weight by Forearm with ggplot2")+
